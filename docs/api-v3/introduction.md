...
All requests must have the `User-Agent` header with the information about the name of your application and aÂ technical contact email so we can contact you in case something goes rogue with your API integration.
...
Unless stated otherwise, data (payload) is sent inside the request/response body as an UTF-8 encoded JSON string. Requests that send JSON payload need toÂ have aÂ header `Content-Type: application/json`.

Requests requesting aÂ different type of response will receive `415 Unsupported Media Type` error.
...
Aside from `2xx` success HTTP status codes, the API may also return `4xx` client and `5xx` server error responses. In case of an error response body always contains aÂ JSON with the description of the error.
...
It contains `errors` key with aÂ map of fields an their errors. The second format is used for all other errors and always contains `error` key with aÂ string code of the error and `error_description` with aÂ human readable description of the error:
...
Your application should always handle errors returned byÂ the API. In cases of aÂ maintenance or aÂ `5xx` server error, your application should retry the request later as Fakturoid does not store the requests for the later processing.

In case of `4xx` client error, your application should display the error message toÂ the user or your integration should be updated in aÂ way toÂ fix the error.
