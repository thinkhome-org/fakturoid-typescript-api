...
Filter moves byÂ inventory item ID
...
[
  ...
  },
  {
    ...
    "retail_currency": "CZK",
    "native_retail_price": null,
    "private_note": "3 letÃ¡ zÃ¡ruka zaÂ pÅÃ­platek.Â Disk jeÂ vybaven nejnovÄjÅ¡Ã­m ÅadiÄem, kterÃ½Â umoÅ¾Åuje dosahovat rychlosti ÄtenÃ­ aÅ¾ 500 MB/s aÂ zÃ¡pisu 320 MB/s.",
...
- If inventory move is successfully created the server will respond with `201 Created` and aÂ JSON body with its data. AÂ `Location` header will also be returned which contains aÂ link toÂ the newly created move.
...
- If the inventory move is successfully updated the server will respond with `200 OK` and aÂ JSON body with its data.
- Request with invalid data will result in response `422 Unprocessable Content` with aÂ JSON body describing errors found in the request.
- When editing aÂ move that is assigned toÂ aÂ document, you will receive `403 Forbidden` from the server.
...
After deleting aÂ move the server will respond with `204 No Content`.

When deleting aÂ move that is assigned toÂ aÂ document, you will receive `403 Forbidden` from the server.
