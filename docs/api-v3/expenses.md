...
Subject registration number (IÄO)
...
Remind the upcoming due date with aÂ Todo  
Default: `true`
...
The price in the expense line is entered without VAT and the VAT is calculated automatically as aÂ percentage from the line
...
[
  {
    ...
    "supplier_street": "KlimentskÃ¡ 1216/46",
    "supplier_city": "Praha",
    "supplier_zip": "11000",
    "supplier_country": "CZ",
...
Following fields are being searched: `number`, `variable_symbol`, `original_number`, `supplier_name`, `supplier_registration_no`, `supplier_vat_no`, `description`, `private_note` and `lines`. Search byÂ tags is done via `tags` query parameter.
