...
*   Every account can create webhooks in their account in Fakturoid UI. How toÂ create webhooks in your account see the [support page (cz)](/podpora/automatizace/webhooky).
*   In addition toÂ the UI, new webhooks can also be created and managed via API using both the [authorization code flow](/api/v3/authorization#authorization-code-flow) and the [client credentials flow](/api/v3/authorization#client-credentials-flow):
*   **Authorization code flow**
    *   ToÂ manage webhooks via API, please [contact our support team](mailto:podpora@fakturoid.cz) first. Without prior authorization, aÂ `Status 403` response will be returned
*   **Client credentials flow**
    *   Webhook management must be enabled in the UI in your user screen _Settings â User account_.
*   Both OAuth integrations and client credentials can manage only their own webhooks; access toÂ other webhooks within the account is restricted.
*   When an OAuth integration or client credentials are revoked, all associated webhooks are automatically deleted.

### [Events](#events)

Webhooks are triggered when one of resources listed below is modified. Action may be triggered byÂ user interaction, API call, or automatically (e.g. invoice is marked as overdue). Some events may include additional payload, which contains related resources toÂ the event. For more information about the payload, please refer toÂ the section on [Payload Content](#payload-content).
...
Invoice was created. Event is triggered automatically, including when invoice is created byÂ recurring generator or as aÂ result of paying proforma.
...
Invoice was deleted and moved toÂ the trash.
...
Payment was added toÂ document and marked it as paid. Payload contains `payment` data.

`invoice_payment_added`

`payment: Payment`

Payment was added toÂ the invoice, but didn't mark it as paid. Payload contains `payment` data.
...
Email with the invoice was sent toÂ the subject.
...
Generator was deleted and moved toÂ the trash.
...
Recurring generator was deleted and moved toÂ the trash.
...
Expense was deleted and moved toÂ the trash.
...
Payment was added toÂ the expense, but the document is still not marked as paid.
...
OCR processing was rejected byÂ external service.
...
*   After successful webhook creation, you will receive aÂ `201 Created` response from the server, the `location` header will be set toÂ the address of the newly created webhook.
...
*   If webhook is successfully updated the server will respond with `200 OK` and aÂ JSON body with its data.
...
Returns aÂ list of webhook events that completely failed toÂ be delivered after all retry attempts, along with details of each delivery attempt.
...
Each item in the response array represents aÂ webhook event that failed toÂ be delivered. The `deliveries` array contains individual delivery attempts sorted from the most recent toÂ the oldest.
...
Content type of the response (`null` if noÂ response was received)

`deliveries[].response_body`

`String`

Body of the response (`null` if noÂ response was received)
...
Webhooks are delivered as HTTP POST requests containing JSON payload toÂ the URL specified in the webhook configuration. Optionally, `Authorization` header is set toÂ the value specified in the `auth_header` attribute of the webhook.

Each delivery includes an `Idempotency-Key` header containing aÂ UUID v7. This key uniquely identifies the webhook delivery and can be used toÂ ensure idempotent processing on the receiver side. The key remains the same across retries of the same webhook delivery.

Receiver must acknowledge the delivery byÂ responding with aÂ status code 2xx within 30 seconds. Otherwise the webhook delivery will be retried up toÂ 5 times with an exponential backoff.
...
The body contains aÂ resources related toÂ event. For instance, when an invoice is created, the body contains an `invoice` key with the serialized [Invoice](/api/v3/invoices#attributes) object. When additional attributes are listed in the table, they are also included in the body object. More information may be found in the attributes section of API documentation related toÂ the resource.

For example, when an invoice is paid, aÂ payment object is included in the body object. Thus JSON contains an `invoice` object and aÂ `payment` object under the `body` key.
...
When payload relates toÂ bulk action, additional attributes are included under pluralized resource name. For example, when all inbox files are removed, the payload contains an `inbox_files` key with number of removed records.
...
### [You can use one of the following services toÂ test webhooks:](#you-can-use-one-of-the-following-services-to-test-webhooks)
