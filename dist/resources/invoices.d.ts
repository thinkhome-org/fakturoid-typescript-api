...
import type { DateString, FakturoidAuth, FakturoidDocumentAttachment, FakturoidDocumentAttachmentInput, FakturoidDocumentLine, FakturoidIbanVisibility, FakturoidLanguage, FakturoidOssMode, FakturoidPaymentMethod, FakturoidVatPriceMode, FakturoidVatRate, FakturoidVatRateSummary, InvoiceDocumentType, InvoiceStatus, MoneyAmount, PaginationOptions } from '../types/common';
...
export type UpdateInvoice = Partial<Omit<NewInvoice, 'number_format_id' | 'tax_document_ids' | 'correction_id' | 'bank_account_id'>>;
/**
 * Invoice action events that can be fired.
 * @see https://www.fakturoid.cz/api/v3/invoices#invoice-actions
 */
export type InvoiceActionEvent = 'mark_as_sent' | 'cancel' | 'undo_cancel' | 'lock' | 'unlock' | 'mark_as_uncollectible' | 'undo_uncollectible';
export declare class InvoicesResource {
     ...
     * @see https://www.fakturoid.cz/api/v3/invoices#fulltext-search
     */
    search(options?: InvoiceSearchOptions): Promise<Invoice[]>;
    get(id: number): Promise<Invoice>;
    create(data: NewInvoice): Promise<Invoice>;
    update(id: number, data: UpdateInvoice): Promise<Invoice>;
    delete(id: number): Promise<void>;
    /**
     * Fire an invoice action event (e.g. mark_as_sent, lock, cancel).
     * @see https://www.fakturoid.cz/api/v3/invoices#invoice-actions
     */
    fireEvent(id: number, event: InvoiceActionEvent): Promise<void>;
    /**
     ...
     */
    downloadPdf(id: number): Promise<ArrayBuffer | null>;
    /**
     ...
     */
    downloadAttachment(invoiceId: number, attachmentId: number): Promise<ArrayBuffer | null>;
    /**
     * Async generator that iterates through all pages of invoices.
     * Yields individual invoices. Use with `for await...of`.
     */
    listAll(options?: Omit<ListInvoicesOptions, 'page'>): AsyncGenerator<Invoice, void, undefined>;
}
